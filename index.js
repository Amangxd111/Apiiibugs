...
const limitFile = path.join(__dirname, 'command_limit.json');
...
app.post('/api/resetlimit', async (req, res) => {
  try {
    const limitPath = path.join(__dirname, 'command_limit.json');
...
Log otomatis oleh sistem monitoring MalzXyz API.
  ...
  try {
    for (let i = 1; i <= 1; i++) {
      ...
      console.log(`(${i}/15) Berhasil kirim bug ke ${jid}`);
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
