...
// Menggunakan node-fetch untuk upload file ke Telegram karena axios tidak mendukung FormData di semua lingkungan
...
// CATATAN: Pastikan Anda telah menginisialisasi FormData global di lingkungan Anda (misalnya, dengan require('form-data') jika di Node.js lama)
...
function getSessionWaNumber(sessionId) {
    const mapping = loadIdMapping();
    for (const number in mapping) {
        if (mapping[number].currentSessionId === sessionId) {
            return mapping[number].phoneNumber;
        }
    }
    return null;
}
...
/**
 * DIPERBAIKI: Memastikan target_number disertakan dalam setiap objek detail dan menambahkan waNumber.
 */
function getBroadcastStatsBySessionId(sessionId) {
    if (!fs.existsSync(LOG_FILE)) return { sent: 0, failed: 0, total: 0, details: [], sessionId: sessionId, waNumber: null };
    let logs = {};
    try { logs = JSON.parse(fs.readFileSync(LOG_FILE, 'utf-8')); } catch(e) { logs = {} }
    ...
    sessionLogs.forEach(log => {
        ...
        details.push({
            target_number: String(log.target), 
            status: log.status,
            reason: log.reason,
    ...
    const waNumber = getSessionWaNumber(sessionId);

    return { sessionId, sent, failed, total: sessionLogs.length, details, waNumber };
}
...
/**
 * FORCE LOGOUT: Mengatur status ke "Diblokir" (untuk Timer/Revoke) atau "Disconnect" (untuk Error/Logout).
 */
    ...
    client.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                ...
                // Menggunakan privacy settings untuk mendeteksi Timer
                ...
                // Jika timerDuration AKTIF (bukan OFF dan > 0), maka diblokir
...
async function sendBackupToTelegram(filePath) {
    // Memeriksa jika variabel konfig Telegram adalah placeholder
    if (TELEGRAM_BOT_TOKEN === '7988999694:AAHrCuGm1SqJUjaBb7ZX2zvNm4Aa_WSOki4' || TELEGRAM_CHAT_ID === '6897791527') {
        console.warn("Telegram configuration is not set up. Skipping backup.");
        return;
    }
    
    // Pastikan FormData tersedia (misalnya, jika lingkungan Node.js mendukungnya)
    if (typeof FormData === 'undefined') {
        console.error("FormData is not available. Cannot upload backup.");
        return;
    }
    ...
    try {
        ...
        // Menggunakan Blob atau Buffer untuk membuat file, tergantung lingkungan
        // Di lingkungan Node.js, Buffer sudah cukup, tapi kita gunakan pendekatan cross-platform
        ...
        const response = await fetch(apiUrl, {
            method: 'POST',
            body: form,
            // Header Content-Type akan otomatis di-set oleh FormData
        });
        ...
        console.log(`[BACKUP] Successfully sent ${fileName} to Telegram.`);
    } catch (error) { 
        console.error(`[BACKUP] Failed to send ${fileName} to Telegram:`, error.message);
    }
}

function setupCronJob() {
    // Jadwal setiap hari pukul 00:00 (tengah malam)
    cron.schedule('0 0 * * *', async () => {
        console.log('[CRON] Starting daily file backup...');
        for (const f of BACKUP_FILES) {
            if (fs.existsSync(f)) {
                await sendBackupToTelegram(f);
            }
        }
...
app.post("/api/pair", async (req, res) => {
    ...
    const mapping = loadIdMapping();
    // Cek status selain Logged Out, Disconnect, atau Diblokir
    ...
    try {
        ...
        console.error(`[API/PAIR] Error during pairing: ${err.message}`);
        sessionsMap.delete(sessionId);
        const sessionPath = path.join(SESSION_DIR, sessionId);
...
app.get("/api/sessions", (req, res) => {
    ...
    for (const number in mapping) {
        ...
        if (entry.status === "Pairing") {
            // Cek Pairing timeout (5 menit)
            if (now - new Date(entry.lastUpdated).getTime() > 300000) { 
        ...
        // Update status jika client live
        if (client && client.authState.creds.registered) {
            displayStatus = "Connected";
        } else if (displayStatus === "Connected") {
            // Jika mapping Connected tapi client tidak ada, anggap Reconnecting/Disconnect
            displayStatus = "Reconnecting"; 
        }

        // Hanya tampilkan status relevan
    ...
    // Urutkan status
    active.sort((a, b) => {
        const order = { "Connected": 1, "Pairing (Active)": 2, "Reconnecting": 3, "Diblokir": 4, "Disconnect": 5 };
...
app.post("/api/save-message", (req, res) => {
    ...
    res.json({ status: true, message: "Message configuration saved." });
});
...
app.post("/api/cek-terkirim", (req, res) => {
    ...
    res.json({ 
        ...
        // Array history (details) kini memiliki field target_number
...
app.post("/api/cancel-broadcast", (req, res) => {
    ...
    } else res.status(404).json({ status: false, message: "Not running." });
});
...
app.listen(PORT, async () => {
    ...
    setupCronJob(); // Setup cron job setelah server listen
}); 