...
app.post('/api/send-sms', async (req, res) => {
  const { number, message, image } = req.body; // Tambahkan image sebagai opsional input
  ...
  try {
    if (image) {
      // Kirim sebagai pesan gambar jika URL gambar tersedia
      await client.sendMessage(jid, {
        image: { url: image },
        caption: message
      });
      console.log(`[+] Gambar berhasil dikirim ke ${jid}: ${message} (${image})`);
